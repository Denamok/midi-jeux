...
# Hook Plugins
eval($plxAdmin->plxPlugins->callHook('AdminTypesPrepend'));
...
<form action="types.php" method="post" id="form_types">

	<div class="inline-form action-bar">
		<h2><?php echo L_TYP_TITLE ?></h2>
		...
		&nbsp;&nbsp;&nbsp;
		<input type="submit" name="update" value="<?php echo L_TYP_APPLY_BUTTON ?>" />
	</div>

	<?php eval($plxAdmin->plxPlugins->callHook('AdminTypesTop')) # Hook Plugins ?>
		...
		<table id="categories-table" class="full-width">
			<thead>
				<tr>
					...
					<th><?php echo L_TYP_LIST_ID.' '.L_TYPE ?></th>
					<th><?php echo L_TYP_LIST_NAME ?></th>
					<th><?php echo L_TYP_LIST_URL ?></th>
					<th><?php echo L_TYP_LIST_ACTIVE ?></th>
					<th><?php echo L_TYP_LIST_SORT ?></th>
					<th><?php echo L_TYP_LIST_BYPAGE ?></th>
					<th><?php echo L_TYP_LIST_ORDER ?></th>
					<th><?php echo L_TYP_LIST_MENU ?></th>
					<th>&nbsp;</th>
				</tr>
			</thead>
			<tbody>
			...
			# Si on a des types
			if($plxAdmin->aTypes) {
				foreach($plxAdmin->aTypes as $k=>$v) { # Pour chaque type
					$ordre = ++$num;
					echo '<tr>';
					...
					echo '</td>';
					echo '<td><a href="type.php?p='.$k.'">'.L_OPTIONS.'</a></td>';
					echo '</tr>';
				}
				# On récupère le dernier identifiant
				$a = array_keys($plxAdmin->aTypes);
				rsort($a);
			} else {
				$a['0'] = 0;
			}
			...
			?>
				<tr class="new">
					<td>&nbsp;</td>
					<td><?php echo L_NEW_TYPE ?></td>
					<td>
					<?php
						echo '<input type="hidden" name="catNum[]" value="'.$new_catid.'" />';
						plxUtils::printInput($new_catid.'_template', 'type.php', 'hidden');
...
<?php
# Hook Plugins
eval($plxAdmin->plxPlugins->callHook('AdminTypesFoot'));
# On inclut le footer
